...
<waf:html>
<head>
	<title>${entityName}UpdateContinue</title>
	...
	<script language="javascript">
		...
		function doSave() {
	        if(doValidate()){
			    ...
			    generalSubmit(document.forms[0], "./apiKeyUpdateSave.do");		
	        }
			return false;
		}
		...
		function doSubmit() {
			if(doValidate()){
				var createdApiIds = document.all('createdApiIds');
	        	setAllOptionsSelected(createdApiIds, true);
			    generalSubmit(document.forms[0], "./apiKeyUpdateSubmit.do");
			}
			return false;
		}
		...
		function doCancel(){
			var okCallback = function() {
				generalSubmit(document.forms[0],  "./apiKeyUpdateCancel.do");
			};
	    ...
	    function doValidate() {
			...
			if (isEmpty(document.all["voApiKey.applicationId"].value)) {
				addErrorMessage("js.pub.required","<waf:label key="voApiKey.label.applicationId"/>");
			}
			if(hasMessages()) {
				showMessages();
				return false;
					...
					<td valign="top">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								...
								<table width="100%" border="0" cellspacing="0" cellpadding="0">
									<tr>
										<td class="info"><waf:label key="apiKey.apiKeyUpdateContinueTitle" /></td>
